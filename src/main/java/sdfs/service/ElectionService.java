...
public class ElectionService {
    ...
    /**
     * Send ElectionMessage over TCP to clientSocket
     *
     * @param clientSocket
     * @param msg
     * @return
    ...
    private String sendMessage(Socket clientSocket, String msg) {
        ...
        try {
            ...
            System.out.println("[DEBUG][Election]: Sent ElectionMessage " + msg);
            response = inFromServer.readLine();
        } catch (IOException e) {
            e.printStackTrace();
            System.out.println("[ERROR][Election]: Sending ElectionMessage " + msg);
        }
        return response;
    }
    ...
    private void setInitialMaster() {
        ...
        communicate(ServerMain.intro_address, ServerMain.intro_port + ServerMain.ESPortDelta, msg);
    }
    ...
    private void handleMessage(String msg, Socket clientSocket) throws IOException {
        System.out.println("[DEBUG][Election]: Recived ElectionMessage " + msg);
        ElectionMessage m = ElectionMessage.extractMessage(msg);
        if (m.type == ElectionMessageType.MASTER_REPLY) {
            String new_master = m.messageParams[0];
            if (!new_master.equals("NOT_SET")) {
                master = new_master;
            }
        } else if (m.type == ElectionMessageType.COORDINATOR) {
            String new_master = m.messageParams[0];
            master = new_master;
            String[] files = ServerMain.FS.getFilesInServer().toArray(new String[0]);
    ...
    private void multicastMaster() {
        ...
        for (String m : memlist) {
            ...
            Pid p = Pid.getPid(m);
            communicate(p.hostname, p.port + ServerMain.ESPortDelta, msg);
        }
        ...
        while (true) {
            try {
                ...
                if (needElection() && isPotentialMaster()) {
                    ...
                    ServerMain.launchMaster(ServerMain.FS.getFilesInServer());
