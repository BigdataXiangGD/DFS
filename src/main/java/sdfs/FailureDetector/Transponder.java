...
import sdfs.message.FDMessageType;
...
public class Transponder extends Thread {
     ...
     * @param m
     */
    private void processInfoPackets(FDMessage m) {
    ...
    public void handleMsg(DatagramPacket receivePacket) {
        ...
        // if ack
        System.out.println("[RECEIVER] [INFO] [" + System.currentTimeMillis() + "] ElectionMessage Received : "
                + new String(receivePacket.getData(), 0, receivePacket.getLength()));

        FDMessage m = FDMessage.extractMessage(receivePacket.getData(), receivePacket.getLength());
        if (m.type == FDMessageType.PING) {
            this.processInfoPackets(m);
            byte[] sendBytes;
            if (membershipSet.contains(m.getMessageSenderID())) {
                sendBytes = FDMessage.MessageBuilder
        ...
        } else if (m.type == FDMessageType.ACK) {
            this.processInfoPackets(m);
        ...
        } else if (m.type == FDMessageType.PING_REQUEST) {
            if (m.getReqDestination().equals(idString)) {
                byte[] sendBytes = FDMessage.MessageBuilder
        ...
        } else if (m.type == FDMessageType.ACK_REQUEST) {
        ...
        } else if (m.type == FDMessageType.MISSING_NOTICE) {
            terminate();
        } else {
            throw new IllegalArgumentException("ElectionMessage type not recognized");
        }
    }
