...
public class InputProcessorThread extends Thread{
	...
	private void processUserCommand(String line) {
        if (line.equals("m")) {
        	...
        	List<String> memlist = ServerMain.FD.getMemlistSkipIntroducer();
        	for(String member: memlist){
        		System.err.println(member);
        	}
        	String intro= ServerMain.FD.getIntroID().toString();
        	if(ServerMain.FD.isAlive(intro)){
        		System.err.println(intro);
        	}
        } else if (line.equals("i")) {
        	System.err.println("SELF ID");
        	System.err.println(ServerMain.FD.getSelfID().toString());
        } else if (line.equals("l")) {
        	System.err.println("Leave Initiated");
        	ServerMain.FD.leaveInitiate();
        }else if (line.equals("b")){
        	System.err.println("Master");
        	System.err.println(ServerMain.ES.getMaster());
        }else if (line.equals("f")){
        	System.err.println("Files Stored");
        	for (String filename : ServerMain.FS.getFilesInServer()){
